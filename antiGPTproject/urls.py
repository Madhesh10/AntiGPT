...
def _make_json_safe(obj):
    ...
    - Path-like -> string
    - tuple -> list
    - other -> str(obj) as fallback for safety
    """
    import pathlib
    ...
    # common Django types: QuerySet, Model, etc. fallback to string
    try:
        # attempt to iterate if it's a list-like
        iter(obj)
    except TypeError:
        return str(obj)
    # final fallback: convert items if iterable
    try:
        return [ _make_json_safe(x) for x in obj ]
    except Exception:
        return str(obj)
